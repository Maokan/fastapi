...
def infos(session:Session = Depends(get_session),user_id: int = Depends(decrypt_token)):
...
def beneficiaries(session = Depends(get_session),user_id: int = Depends(decrypt_token)) -> list[Beneficiary]:
    beneficiaries = session.exec(select(Beneficiary).join(Account).where(Account.user_id == user_id)).all()
...
async def send(body: GetSendInformation, session = Depends(get_session), user_id: int = Depends(decrypt_token)):
...
def closeAccount(body: CloseAccount, session = Depends(get_session),user_id: int = Depends(decrypt_token)):
    account = session.get(Account,body.account_id)
...
def openAccount(body: CreateAccount, session=Depends(get_session),user_id: int = Depends(decrypt_token)):
    ...
    if body.type == "Principal":
        existing_account = session.exec(select(Account).where(Account.user_id == user_id, Account.type == "Principal")).first()
        if existing_account:
    ...
    account = createAccount(user_id, body.type, session)
    return {"SUCCESS": f"Compte ouvert avec l'id : {account.id}, Numéro de compte : {account.account_number}"}

@app.post("/add-beneficiary")
def beneficiary(body: CreateBeneficiary, session=Depends(get_session),user_id: int = Depends(decrypt_token)):
...
def login(body:GetUser, session:Session = Depends(get_session)):
    ...
    user_id = mail.id
    return {"token": generate_token(mail)}
